...
void loop() // Main loop auto-repeats
{
  ...
  //goal is to take a lot of measurements and then take the average
  unsigned int IRL = 0;  //temp IR Left
  unsigned int IRR = 0;  //temp IR Right
  unsigned int counter = 100;
  
  unsigned int irLeft;
  unsigned int irRight;
  
  for (unsigned int i = 0; i < counter; i++)
  {
    irLeft = digitalRead (IRRECEIVERPIN1);
    delay(1);
    irRight = digitalRead (IRRECEIVERPIN2);
    IRL = irLeft + IRL;
    IRR = irRight + IRR;
  }
